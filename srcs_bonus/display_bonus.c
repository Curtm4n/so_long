...
/*   Updated: 2021/09/06 15:12:11 by cdapurif         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_player(t_info *info, int key)
{
	int			check;
	static int	move = 0;
	...
	if (key == 'a' || key == 65361)
	{
		info->pl_texture->texture = info->left_texture->texture;
		move += move_x(info, -1);
	}
	if (key == 's' || key == 65364)
		move += move_y(info, 1);
...
int	my_key_hook(int key, void *param)
{
	...
	if ((key > 65360 && key < 65365) || key == 'w' || key == 'a' || key == 's' \
		|| key == 'd')
	{
		move = move_player((t_info *)param, key);
		if (info->collectible == 0)
		{
			free_texture(info->mlx_ptr, info->ex_texture);
			info->ex_texture = get_texture("./res/exit_open.xpm", info);
		}
		mlx_clear_window(info->mlx_ptr, info->window_ptr);
		display_map(info, move);
	}
	return (0);
}
